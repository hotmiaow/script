...
CATEGORIES = {
    ...
    "13_replacemsg": [r"^config system replacemsg"],
}
...
def write_category(output_dir, cat_prefix, blocks, max_lines=3000):
    # blocks is now a list of tuples: (vdom_name, block_lines)
    for vdom, block_lines in blocks:
        if not block_lines:
            continue
        ...
        # Clean VDOM name to ensure valid filenames
        clean_vdom = re.sub(r'[\\/*?:"<>|]', "", vdom)
        prefix = f"{clean_vdom}_{cat_prefix}" if clean_vdom != "global" else cat_prefix

        # Prepare VDOM wrappers to ensure the extracted configs are valid for import
        wrapper_head = []
        wrapper_tail = []
        if clean_vdom != "global":
            wrapper_head = ["config vdom\n", f"edit {vdom}\n"]
            wrapper_tail = ["end\n"]
        ...
        if not edits or len(block_lines) <= max_lines:
            ...
            for i, chunk in enumerate(chunks):
                filename = f"{prefix}_{name}_part{i+1}.txt"
                with open(os.path.join(output_dir, filename), "w", encoding="utf-8") as f:
                    f.writelines(wrapper_head)
                    f.writelines(header)
                    for edit in chunk:
...
def split_fortigate_config(input_file: str, max_lines: int):
    ...
    for line in lines:
        ...
        # 1. Update Global Config Stack to track hierarchy
        if stripped.startswith("config "):
            config_stack.append(stripped)
        elif stripped == "end" and config_stack:
            config_stack.pop()

        # 2. Track current VDOM context
        if len(config_stack) == 1 and config_stack[0] == "config vdom" and stripped.startswith("edit "):
            current_vdom = stripped.replace("edit ", "").strip(' "\'\n')
        elif len(config_stack) == 0:
            current_vdom = "global"

        # 3. Capture logic for specific blocks
        if capture_target:
            ...
            if stripped.startswith("config "):
                ...
                if capture_depth == 0:
                    if capture_target == "interface_split":
                        interface_blocks.append((current_vdom, capture_lines))
                    else:
                        blocks_to_write[capture_target].append((current_vdom, capture_lines))
                    capture_target = None
                    capture_lines = []
            continue
    ...
    for vdom, block_lines in interface_blocks:
        header, edits, footer = split_block_into_edits(block_lines)
        lag_edits = []
        iface_edits = []
        for e in edits:
        ...
        if lag_edits:
            ...
            blocks_to_write["01_LAG"].append((vdom, header + flat_lag + footer))

        if iface_edits:
            flat_iface = [l for e in iface_edits for l in e]
            blocks_to_write["02_interface"].append((vdom, header + flat_iface + footer))

        if not lag_edits and not iface_edits:
            blocks_to_write["02_interface"].append((vdom, block_lines))
