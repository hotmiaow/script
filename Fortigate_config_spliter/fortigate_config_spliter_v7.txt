...
def write_category(output_dir, cat_prefix, blocks, max_lines=3000):
    if cat_prefix == "13_replacemsg":
        if blocks:
            filename = f"{cat_prefix}_all.txt"
...
def split_fortigate_config(input_file: str, max_lines: int, keep_snmp_index: bool):
    ...
    capture_lines = []
    capture_depth = 0
    snmp_removed_count = 0

    for line in lines:
        stripped = line.strip()
        
        if not keep_snmp_index and stripped.startswith("set snmp-index"):
            snmp_removed_count += 1
            continue
    ...
    if not keep_snmp_index:
        print(f"Removed {snmp_removed_count} 'set snmp-index' lines.")
...
def main():
    ...
    parser.add_argument("--keep-snmp", action="store_true",
                        help="Keep 'set snmp-index' lines in the output (they are removed by default)")

    args = parser.parse_args()
    split_fortigate_config(args.config, args.max_lines, args.keep_snmp)
