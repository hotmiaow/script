...
def process_edit_block(config_type, name, lines):
    ...
    if subnets:
        # Ignore 0.0.0.0 0.0.0.0 (all) and 0.0.0.0 single host wildcard mappings
        filtered_subnets = [n for n in subnets if str(n) not in ('0.0.0.0/0', '0.0.0.0/32')]
        if filtered_subnets:
            return IpObject(name, config_type, lines, filtered_subnets)
    return None
...
def find_unused_ips(target_subnets, ip_objects, ignore_supernets=False):
    ...
    overlapping_nets = []
    ignored_supernets = False
    for obj in ip_objects:
        for net in obj.subnets:
            if ignore_supernets:
                # Ignore networks that completely cover or are exactly equal to our target
                is_supernet = False
                for t_net in target_subnets:
                    if t_net.network_address >= net.network_address and t_net.broadcast_address <= net.broadcast_address:
                        is_supernet = True
                        break
                if is_supernet:
                    ignored_supernets = True
                    continue
    ...
    if ignored_supernets:
         print("   (Note: Successfully ignored exact match / wrapper subnets for this calculation.)")
    ...
    used_ips = set()
    # Math check ip in net is O(1) in python.
...
def main():
    ...
    if not overlapping_objects:
        ...
        for obj in overlapping_objects:
            if obj.obj_type in summary_info:
                # Include the subnets string next to the object name
                subnets_str = ", ".join(str(n) for n in obj.subnets)
                summary_info[obj.obj_type].append(f"{obj.name} ({subnets_str})")
    ...
    # Passing tightly coupled ignore_supernets=True solves exactly the issue with the user wanting
    # to find individual unused IPs even if the exact /24 target exists in the FW.
    unused_ranges = find_unused_ips(target_subnets, ip_objects, ignore_supernets=True)
    if not unused_ranges:
        print(f"All IPs within {target_str} are CURRENTLY IN USE natively.")
    else:
        print(f"The following IP(s) within {target_str} are NOT in use yet (ignoring exact/larger wrapper subnets):")
