...
class CiscoTracerouteMapper:
        ...
        try:
            with open(self.csv_file, encoding="utf-8") as fh:
                ...
                for row in reader:
                    ...
                    self.ip_to_device_map[ip] = (device, iface, zone, vrf)
    ...
    def _display_hop(self, hop_no: int, ip: Optional[str]) -> None:
        ...
        device, iface, zone, vrf = self.ip_to_device_map.get(ip, (None, None, None, None))
        ...
        if device:
            ...
            print(
                f"{hop_no:>2}   {show_ip:<40}  {device:<20} {iface:<20} {zone:<15} {vrf}{extra}"
            )
        else:
    ...
    def display_device_inventory(self) -> None:
        ...
        for zone in sorted(zones):
            ...
            for dev in sorted(zones[zone]):
                print(f"\n📍 {dev}")
                print(f"{'Interface':<25} {'IP Address':<16} {'Zone':<15} {'VRF'}")
                print("-" * 70)
                for iface, ip, z, v in self.device_inventory[dev]:
                    print(f"{iface:<25} {ip:<16} {z:<15} {v}")
        ...
        for h in range(1, max_hop + 1):
            ...
            for i, (_, target_ip) in enumerate(resolved_targets):
                ...
                if hop_ip is None:
                    ...
                    device, iface, _, _ = self.ip_to_device_map.get(hop_ip, (None, None, None, None))
