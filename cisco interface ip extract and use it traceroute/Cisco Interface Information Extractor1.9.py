...
class NetworkConfigParser:
    ...
    def extract_all_zone_configs(self, content: str, zones: Dict[str, str]) -> None:
        ...
        print(f"DEBUG_TIMING: extract_all_zone_configs regex search starting (len: {len(content)})...")
        matches = re.findall(zone_interface_pattern, content, re.DOTALL | re.IGNORECASE)
        print(f"DEBUG_TIMING: extract_all_zone_configs regex search complete. Found {len(matches)} potential matches.")
