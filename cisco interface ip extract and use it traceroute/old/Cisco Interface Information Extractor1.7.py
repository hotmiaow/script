...
class NetworkConfigParser:
    ...
    def extract_all_zone_configs(self, content: str, zones: Dict[str, str], filename: str = "") -> None:
        """Fallback method to find any missed zone configurations using line-based parsing"""
        print(f"DEBUG_TIMING: extract_all_zone_configs line-based search starting (len: {len(content)})...")
        
        lines = content.split('\n')
        total_lines = len(lines)
        current_zone = None
        
        # Regex for line-by-line processing
        edit_zone_pattern = re.compile(r'^\s*edit\s+"?([^\s"\n\r]+)"?', re.IGNORECASE)
        set_interface_pattern = re.compile(r'^\s*set\s+interface\s+([^\n\r]+)', re.IGNORECASE)
        ...
        while i < len(lines):
            ...
            if i % 10000 == 0:
                 percent = (i / total_lines) * 100
                 print(f"DEBUG_TIMING: Progress (Fallback): [{filename}] Processed: {i} / Total: {total_lines} ({percent:.1f}%)")

            # Check for "edit <ZONE>"
            # Note: This is context-unaware (aggressively looks for edit commands)
            # This is by design for the "fallback" method
            edit_match = edit_zone_pattern.match(line)
            if edit_match:
                current_zone = edit_match.group(1).strip('"')
            
            # Check for "set interface <IFACE>"
            if current_zone:
                interface_match = set_interface_pattern.match(line)
                if interface_match:
        ...
        print(f"DEBUG_TIMING: extract_all_zone_configs search complete.")
    ...
    def parse_fortigate_interfaces(self, content: str, hostname: str, filename: str = "") -> List[Dict]:
        ...
        if not interface_match:
            ...
            # Check if file has .set extension
    ...
    def extract_fortigate_zones(self, content: str, filename: str = "") -> Dict[str, str]:
        ...
        self.extract_all_zone_configs(content, zones, filename)
