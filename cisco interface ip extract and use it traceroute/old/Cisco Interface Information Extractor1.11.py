...
class NetworkConfigParser:
    ...
    def extract_vdom_zones(self, content: str, zones: Dict[str, str]) -> bool:
        ...
        # Updated pattern to capture optional 'edit <NAME>' in the same line
        # Matches: "config vdom" or "config vdom edit VDOM_A"
        vdom_start_pattern = r'^\s*config\s+vdom(?:\s+edit\s+"?([^\s"\n\r]+)"?)?\s*$'
        ...
        for vdom_match in vdom_matches:
            ...
            # Check if VDOM name was captured in the config line (condensed syntax)
            condensed_vdom_name = vdom_match.group(1).strip('"') if vdom_match.group(1) else None
            ...
            if condensed_vdom_name:
                # Condensed syntax: config vdom edit NAME
                print(f"    Debug: Processing condensed VDOM '{condensed_vdom_name}'")
                # The section content is the VDOM content itself
                if self.extract_zones_from_content(vdom_section, condensed_vdom_name, zones):
                    found_vdom_zones = True
            else:
                # Standard syntax: config vdom \n edit NAME ...
    ...
    def extract_global_zones_flexible(self, content: str, zones: Dict[str, str], filename: str = "") -> bool:
        ...
        while i < len(lines):
            if i % 2000 == 0:
                 ...
                 print(f"DEBUG_TIMING: Progress: [{filename}] Processed: {i} / Total: {total_lines} ({percent:.1f}%)")
            line = lines[i].strip()
    ...
    def extract_flat_global_zones(self, content: str, zones: Dict[str, str]) -> bool:
        """
        Extract interface-to-zone mappings from flat global configuration.
        Matches: config global config system interface edit "NAME" set interface "ZONE"
        """
        found_zones = False
        print("DEBUG_TIMING: extract_flat_global_zones search starting...")

        # Regex for flat syntax: config global config system interface edit "NAME" set interface "ZONE"
        # Handles optional quotes and whitespace
        flat_pattern = r'^\s*config\s+global\s+config\s+system\s+interface\s+edit\s+"?([^"\n\s]+)"?\s+set\s+interface\s+"?([^"\n]+)"?'
        
        flat_matches = re.finditer(flat_pattern, content, re.MULTILINE | re.IGNORECASE)
        ...
        for match in flat_matches:
            ...
            # Use root as default VDOM for global config, but also could be unknown
            # We'll map to root::interface since that's the most likely target for global config
            # and potentially also map a general case
            
            zone_key_root = f"root::{interface_name}"
            
            # Only add if not already present or override? 
            # Usually specific vdom config takes precedence, but this might be the source of truth for some setups.
            if zone_key_root not in zones:
                zones[zone_key_root] = zone_name
                print(f"      Debug: Flat global found: {zone_key_root} -> {zone_name}")
                found_zones = True
                count += 1
                
            # Also add to unknown for fallback lookup
            zone_key_unknown = f"unknown::{interface_name}"
            if zone_key_unknown not in zones:
                zones[zone_key_unknown] = zone_name
        
        print(f"DEBUG_TIMING: extract_flat_global_zones complete. Found {count} new associations.")
        return found_zones

    def extract_all_zone_configs(self, content: str, zones: Dict[str, str]) -> None:
        """Fallback method to find any missed zone configurations using a line-by-line state machine"""
        # Optimized to avoid catastrophic backtracking from re.DOTALL regex on large files
        
        print(f"DEBUG_TIMING: extract_all_zone_configs state machine search starting (len: {len(content)})...")
        
        current_zone = None
        lines = content.splitlines()
        total_lines = len(lines)
        
        # Regex to match 'edit "ZONE_NAME"' or 'edit ZONE_NAME'
        # Handles optional quotes and flexible whitespace
        edit_pattern = re.compile(r'^\s*edit\s+"?([^\s"]+)"?', re.IGNORECASE)
        
        # Regex to match 'set interface INTERFACE_LIST'
        # Handles optional quotes and flexible whitespace
        interface_pattern = re.compile(r'^\s*set\s+interface\s+(.+)', re.IGNORECASE)
        
        matches_found = 0
        
        for i, line in enumerate(lines):
            # Progress reporting every 10% or at least every 5000 lines
            if i % 5000 == 0 and i > 0:
                 percent = (i / total_lines) * 100
                 print(f"    Scanning line {i}/{total_lines} ({percent:.1f}%)")
            ...
            if edit_match:
                current_zone = edit_match.group(1).strip()
                continue
                
            # Check for 'set interface ...' if we are inside a zone block
            # Note: We assume we are inside a zone block if we saw an 'edit' command recently.
            # This is a 'fallback' heuristic, essentially simulating the previous greedy regex 
            # but in a safe linear scan. 
            if current_zone:
                interface_match = interface_pattern.match(line)
                if interface_match:
                    interface_line = interface_match.group(1)
                    interfaces = self.parse_interface_list(interface_line)
                    ...
                    for iface in interfaces:
                        ...
                        # Only add if not already found (prioritize earlier/more structured findings)
                        if zone_key not in zones:
                            zones[zone_key] = current_zone
                            matches_found += 1
                            # print(f"          Debug: Fallback found: {zone_key} -> {current_zone}")

        print(f"DEBUG_TIMING: extract_all_zone_configs search complete. Found {matches_found} new associations.")
    ...
    def parse_fortigate_interfaces(self, content: str, hostname: str, filename: str = "") -> List[Dict]:
        """Parse Fortigate interface configurations with VDOM and zone support"""
        print(f"DEBUG_TIMING: Start parsing Fortigate interfaces for file '{filename}' (len: {len(content)})")
        interfaces = []

        # Extract zones from the entire config
        print("DEBUG_TIMING: Extracting zones...")
        zones = self.extract_fortigate_zones(content, filename)
        print(f"DEBUG_TIMING: Zones extracted (total: {len(zones)})")
        ...
        if not interface_match:
            print("    No 'config system interface' section found - checking for global flat syntax")

            # Check if file has .set extension - REMOVED RESTRICTION to support more file types
            # if not filename.lower().endswith('.set'):
            #     print(f"    Skipping global flat syntax check for file '{filename}' (not .set extension)")
            #     return interfaces
            pass
            
            # Initialize dictionary for aggregated interface data
            # Key: interface_name, Value: dict of attributes
            global_interfaces = {}
            
            # Regex for flat syntax: config global config system interface edit "NAME" set ...
            print("DEBUG_TIMING: Starting regex search for global flat syntax...")
            flat_pattern = r'^\s*config\s+global\s+config\s+system\s+interface\s+edit\s+"?([^"\n\s]+)"?\s+set\s+(.+)$'
            flat_matches = re.findall(flat_pattern, content, re.MULTILINE | re.IGNORECASE)
            print(f"DEBUG_TIMING: Global flat syntax search complete. Found {len(flat_matches)} matches.")
            
            if flat_matches:
                print(f"    Debug: Found {len(flat_matches)} global interface configuration lines")
                
                for interface_name, set_command in flat_matches:
                    interface_name = interface_name.strip('"')
                    
                    if interface_name not in global_interfaces:
                        global_interfaces[interface_name] = {
                            'vdom': 'root', # Default to root, override if found
                            'ip': None,
                            'cidr': None,
                            'description': 'N/A',
                            'zone': None,
                    ...
                    # Parse set command
                    # set vdom "NAME"
                    vdom_match = re.search(r'vdom\s+"?([^"\n]+)"?', set_command, re.IGNORECASE)
                    if vdom_match:
                        global_interfaces[interface_name]['vdom'] = vdom_match.group(1).strip('"')
                        
                    # set ip IP MASK
                    ip_match = re.search(r'ip\s+(\d+\.\d+\.\d+\.\d+)\s+(\d+\.\d+\.\d+\.\d+)', set_command, re.IGNORECASE)
                    if ip_match:
                        global_interfaces[interface_name]['ip'] = ip_match.group(1)
                        global_interfaces[interface_name]['cidr'] = ip_match.group(2) # Temporarily store mask
                        
                    # set ip IP/CIDR
                    ip_cidr_match = re.search(r'ip\s+(\d+\.\d+\.\d+\.\d+)/(\d+)', set_command, re.IGNORECASE)
                    if ip_cidr_match:
                        global_interfaces[interface_name]['ip'] = ip_cidr_match.group(1)
                        global_interfaces[interface_name]['cidr'] = ip_cidr_match.group(2)
                        
                    # set description "DESC"
                    desc_match = re.search(r'description\s+"?([^"\n]+)"?', set_command, re.IGNORECASE)
                    if desc_match:
                        global_interfaces[interface_name]['description'] = desc_match.group(1).strip()
                        
                    # set alias "DESC" (fallback for description)
                    alias_match = re.search(r'alias\s+"?([^"\n]+)"?', set_command, re.IGNORECASE)
                    if alias_match:
                       if global_interfaces[interface_name]['description'] == 'N/A':
                           global_interfaces[interface_name]['description'] = alias_match.group(1).strip()
                           
                    # set interface "ZONE" (explicit zone mapping)
                    zone_match = re.search(r'interface\s+"?([^"\n]+)"?', set_command, re.IGNORECASE)
                    if zone_match:
                        global_interfaces[interface_name]['zone'] = zone_match.group(1).strip('"')
                        
                    # set mode dhcp
                    if 'mode dhcp' in set_command.lower():
                        global_interfaces[interface_name]['dhcp'] = True
                        
                    # set mode pppoe
                    if 'mode pppoe' in set_command.lower():
                        global_interfaces[interface_name]['pppoe'] = True
                        
                # Convert aggregated data to interfaces list
                for iface_name, data in global_interfaces.items():
                    ip_with_cidr = None
                    if data['ip'] and data['cidr']:
                         mask_or_cidr = data['cidr']
                         if mask_or_cidr.isdigit() and int(mask_or_cidr) <= 32:
                             ip_with_cidr = f"{data['ip']}/{mask_or_cidr}"
                         else:
                             cidr = self.netmask_to_cidr(mask_or_cidr)
                             ip_with_cidr = f"{data['ip']}/{cidr}"
                    elif data['dhcp']:
                        ip_with_cidr = "DHCP"
                    elif data['pppoe']:
                        ip_with_cidr = "PPPoE"
                    ...
                    if ip_with_cidr:
                        ...
                        zone_name = data['zone']
                        if not zone_name:
                            # Try standard zone lookup
                            zone_name = self.get_zone_for_interface(iface_name, data['vdom'], zones)
                            
                        # Format device name
                        if data['vdom'] != "root":
                            device_name = f"{hostname} (VDOM: {data['vdom']})"
                        else:
                        ...
                        print(f"        Found global interface: {iface_name}, IP: {ip_with_cidr}, VDOM: {data['vdom']}, Zone: {zone_name}")
                        
            if not flat_matches and not interface_match:
                 return interfaces

        if interface_match:
             interface_section = interface_match.group(1)
        else:
             # Stop processing if we only found flat matches (or none)
             return interfaces
    ...
    def extract_fortigate_zones(self, content: str, filename: str = "") -> Dict[str, str]:
        """
        ...
        print("DEBUG_TIMING: extract_fortigate_zones -> extract_vdom_zones")
        vdom_zones_found = self.extract_vdom_zones(content, zones)

        # Then, look for global zones (outside VDOMs)
        print("DEBUG_TIMING: extract_fortigate_zones -> extract_global_zones_flexible")
        global_zones_found = self.extract_global_zones_flexible(content, zones, filename)

        # Look for flat global zones
        print("DEBUG_TIMING: extract_fortigate_zones -> extract_flat_global_zones")
        flat_global_zones_found = self.extract_flat_global_zones(content, zones)

        # Also search for any remaining zone configurations that might be missed
        print("DEBUG_TIMING: extract_fortigate_zones -> extract_all_zone_configs")
    ...
    def parse_file(self, filepath: str) -> List[Dict]:
        ...
        if device_type == 'fortigate':
            ...
            # Pass filename for extension check
            filename = os.path.basename(filepath)
            interfaces.extend(self.parse_fortigate_interfaces(content, hostname, filename))
        else:  # Cisco
