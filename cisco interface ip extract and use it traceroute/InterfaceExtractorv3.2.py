...
class NetworkConfigParser:
    def __init__(self):
        self.interface_data = []
        self.device_zones = {}  # normalized_hostname -> { "vdom::interface": zone_name }
        self.canonical_hostnames = {}  # normalized_hostname -> first-seen display hostname

    def _device_key(self, hostname: str) -> str:
        """Normalize a hostname for use as an internal dict key so that cross-file
        correlation (device_zones) isn't broken by hostname casing/whitespace
        differences between split config files for the same device
        (e.g. 'HK1-AAA_root.set' vs 'hk1-aaa_AWS.set')."""
        return (hostname or "").strip().lower()

    def _canonical_hostname(self, hostname: str) -> str:
        """Return a consistent display hostname for a device across all files,
        so split-file output doesn't fragment into differently-cased device names."""
        key = self._device_key(hostname)
        if key not in self.canonical_hostnames:
            self.canonical_hostnames[key] = hostname
        return self.canonical_hostnames[key]
    ...
    def _retroactive_zone_resolution(self, hostname: str) -> None:
        ...
        key = self._device_key(hostname)
        if key not in self.device_zones:
            return
        device_zones = self.device_zones[key]
        canonical = self._canonical_hostname(hostname)

        for item in self.interface_data:
            dev_name = item.get("device_name", "")
            if self._device_key(dev_name) == key or self._device_key(dev_name).startswith(f"{key} (vdom:"):
                if item.get("zone") in ("No Zone", "N/A", ""):
                    ...
                    if new_zone != "No Zone":
                        item["zone"] = new_zone
                        item["device_name"] = f"{canonical} (VDOM: {resolved_vdom})" if resolved_vdom != "root" else canonical
    ...
    def parse_fortigate_interfaces(self, content: str, hostname: str, default_vdom: str = "root", filepath: Optional[str] = None) -> List[Dict]:
        ...
        interfaces = []
        key = self._device_key(hostname)
        display_hostname = self._canonical_hostname(hostname)
        ...
        # Record in device_zones for cross-file correlation (keyed case/whitespace-insensitively
        # so 'HK1-AAA_root.set' and 'hk1-aaa_AWS.set' correlate to the same device)
        if key not in self.device_zones:
            self.device_zones[key] = {}
        self.device_zones[key].update(file_zones)

        zones = self.device_zones[key]
        ...
        for interface_name, interface_config in interface_blocks:
            ...
            for pattern in ip_patterns:
                ...
                for match in ip_matches:
                    ...
                    if active_vdom != "root":
                        device_name = f"{display_hostname} (VDOM: {active_vdom})"
                    else:
                        device_name = display_hostname
            ...
            if not ip_found:
                ...
                if dhcp_match:
                    ...
                    device_name = f"{display_hostname} (VDOM: {vdom_name})" if vdom_name != "root" else display_hostname
    ...
    def parse_cisco_evpn(self, content: str, hostname: str) -> List[Dict]:
        ...
        # NVE interfaces have no IP address of their own but reference a source-interface.
        # parse_cisco_interfaces() already captures every generic "interface X" block
        # (Loopback, Vlan, Tunnel, Port-channel, Management, Mgmt, BVI, NVE, etc.) including
        # their IP addresses, so re-matching those same interfaces here duplicated rows in the
        # output (previously with inconsistent name casing, e.g. "loopback0" vs "Loopback0",
        # which also meant the duplicates weren't being merged). Only the NVE
        # no-IP/source-interface case is unique and not covered by parse_cisco_interfaces.
        nve_pattern = r'interface\s+[Nn][Vv][Ee]\s*(\d+)\s*\r?\n(.*?)(?=^interface\s+|^!\s*$|\Z)'
        for interface_num, interface_config in re.findall(nve_pattern, content, re.MULTILINE | re.DOTALL | re.IGNORECASE):
            interface_name = f"NVE{interface_num}"

            has_ip = re.search(r'ip\s+address\s+\d+\.\d+\.\d+\.\d+', interface_config, re.IGNORECASE)
            if has_ip:
                continue  # already captured generically by parse_cisco_interfaces()
        ...
        for filepath in text_files:
            try:
                ...
                if self.detect_device_type(content, filepath=filepath) == 'fortigate':
                    ...
                    self._canonical_hostname(hname)
                    key = self._device_key(hname)
                    if key not in self.device_zones:
                        self.device_zones[key] = {}
                    self.device_zones[key].update(f_zones)
            except Exception:
                pass
