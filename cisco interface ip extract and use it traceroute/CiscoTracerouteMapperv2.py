...
class CiscoTracerouteMapper:
    ...
    def check_addon_subnet(self, ip_str: str) -> Optional[str]:
        """Checks if IP is in any addon subnet. Returns info string or None."""
        try:
            ip = ipaddress.ip_address(ip_str)
            matches = []
            for net, dev, info in self.addon_subnets:
                if ip in net:
                    matches.append(f"Matches {str(net)} (Behind {dev}) [{info}]")
    ...
    def _display_hop(self, hop_no: int, ip: Optional[str]) -> None:
        ...
        device, iface, zone = self.ip_to_device_map.get(ip, (None, None, None))
        show_ip = f"{ip} ({ptr})" if ptr else ip

        if device:
            print(
                f"{hop_no:>2}   {show_ip:<40}  {device:<20} {iface:<20} {zone}"
            )
        else:
    ...
    def trace_to_destination_streaming(
        ...
        # Check addon info
        addon_info = self.check_addon_subnet(target_ip)
        if addon_info:
            print(f"ℹ️  ADDON INFO: {addon_info}")
    ...
    def display_device_inventory(self) -> None:
        ...
        for zone in sorted(zones):
            ...
            for dev in sorted(zones[zone]):
                ...
                print("-" * 50)
                for iface, ip, z in self.device_inventory[dev]:
                    print(f"{iface:<25} {ip:<16} {z}")
        ...
        for h in range(1, max_hop + 1):
            ...
            for i, (_, target_ip) in enumerate(resolved_targets):
                ...
                if hop_ip is None:
                    ...
                    device, _, _ = self.ip_to_device_map.get(hop_ip, (None, None, None))

                    if device:
                        cell_text = f"{hop_ip} [{device}]"
                    elif ptr:
                        # Shorten PTR if too long
                        short_ptr = ptr.split('.')[0]
                        if len(short_ptr) > 15:
                             short_ptr = short_ptr[:12] + "..."
    ...
    def compare_traces(self, targets: List[str]) -> None:
        ...
        for orig_t, ip in resolved_targets:
            addon_info = self.check_addon_subnet(ip)
            if addon_info:
                 print(f"ℹ️  ADDON INFO for {orig_t} ({ip}): {addon_info}")
    ...
    def find_nodes_by_query(self, query: str) -> Dict[str, List[str]]:
        ...
        if query in self.device_inventory:
            for _, ip, _ in self.device_inventory[query]:
                matches.setdefault(query, []).append(ip)

        # 2. IP match
        if query in self.ip_to_device_map:
            dev, _, _ = self.ip_to_device_map[query]
                ...
                for ip_str in self.ip_to_device_map:
                    try:
                        if ipaddress.ip_address(ip_str) in net:
                            dev, _, _ = self.ip_to_device_map[ip_str]
    ...
    def run_verification_batch(
        ...
        if common_ips:
            ...
            for ip in sorted(common_ips, key=lambda x: ip_sort_key.get(x, 999)):
                dev, _, _ = self.ip_to_device_map.get(ip, (None, None, None))
        ...
        for orig_target, ip in resolved_targets:
            ...
            for h in sorted(hop_map.keys()):
                ...
                # 1. Inventory Check
                dev_at_hop, _, _ = self.ip_to_device_map.get(h_ip_str, (None, None, None))
                if dev_at_hop:
