...
def generate_example_csv(filename):
    with open(filename, mode='w', encoding='utf-8', newline='') as f:
        writer = csv.writer(f)
        # Writing the header as requested
        writer.writerow(['subnet', 'subnet_mask', 'interface', 'gateway', 'comment'])
        # Adding a couple of handy examples
        writer.writerow(['113.12.11.0', '255.255.255.0', 'LAG2.200', '10.0.0.1', 'ZCC_Route'])
        writer.writerow(['113.12.12.0/24', '', 'LAG2.200', '', 'Example_Route_2'])
...
def main():
    ...
    with open(input_file, mode='r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            if ip_col.lower() == 'subnet' and not header_skipped:
                header_skipped = True
                continue
