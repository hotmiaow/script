...
class SearchEngine:
    ...
    def _init_db(self):
        """Initializes database schema with FTS5 support or fallback."""
        conn = self.get_connection()
        cur = conn.cursor()
        
        # Metadata table to track file modifications
        cur.execute("""
    ...
    def search(self, query_str, limit=300):
        """Performs fast search across indexed CSV rows."""
        ...
        try:
            if self.use_fts:
                # Sanitize search term for FTS5 syntax
                clean_query = ' '.join(f'"{token}"*' for token in query_str.split() if token)
                if not clean_query:
                    clean_query = f'"{query_str}"*'
...
class BackgroundIndexer(threading.Thread):
    """Background worker thread to continuously track and index CSV files."""
    ...
    def sync_content_directory(self):
        """Checks for new, updated, or removed CSV files."""
        ...
        if CONTENT_DIR.exists():
            for entry in CONTENT_DIR.rglob("*.csv"):
                try:
                    stat = entry.stat()
        ...
        for rfile in removed_files:
            rel_name = os.path.basename(rfile)
        ...
        if changed_files or removed_files:
            for filepath in changed_files:
                self.index_single_csv(conn, filepath)
    ...
    def index_single_csv(self, conn, filepath):
        """Reads and indexes a single CSV file into SQLite."""
        ...
        try:
            ...
            cur.execute(
                "INSERT OR REPLACE INTO file_meta (file_path, mtime, size, row_count) VALUES (?, ?, ?, ?);",
                (filepath, stat.st_mtime, stat.st_size, total_rows)
            )
            conn.commit()
...
if HAS_TKINTER:
    class QSearchGUIApp:
        ...
        def _setup_ui(self):
            ...
            self.tree.heading("file", text="CSV File")
            self.tree.heading("row", text="Row #")
            self.tree.heading("content", text="Matched Content")
        ...
        def _setup_indexer(self):
            def on_stats_update(file_cnt, row_cnt):
                self._msg_queue.put(("stats", (file_cnt, row_cnt)))
        ...
        def _update_stats_display(self, file_cnt, row_cnt):
            self.lbl_index_stats.config(text=f"Indexed: {file_cnt} file(s), {row_cnt:,} rows | Folder: ./content")
        ...
        def _perform_search(self):
            ...
            results, elapsed_ms = self.engine.search(query, limit=300)
            for fname, rnum, ltext in results:
                self.tree.insert("", "end", values=(fname, rnum, ltext))

            count = len(results)
            limit_notice = " (showing top 300)" if count >= 300 else ""
            self.lbl_status.config(text=f"Found {count} match(es){limit_notice} in {elapsed_ms:.1f} ms for '{query}'")
        ...
        def _on_escape_pressed(self, event=None):
            # 1st ESC press: clears search input & results
            # 2nd ESC press (when already empty): exits the application
            if self.search_var.get().strip() or self.tree.get_children():
                self._clear_search()
            else:
                self.root.destroy()
...
def run_interactive_cli(stdscr):
    ...
    while True:
        ...
        info_str = f" Matches: {len(results)} | Time: {elapsed_ms:.1f}ms | Index: {fc} files ({rc:,} rows) "
        ...
        if results and max_results_rows > 0:
            ...
            for idx, (fname, rnum, ltext) in enumerate(visible_results):
                row_y = start_row + idx
                line_disp = f"[{fname}:L{rnum}] {ltext}"
        ...
        # Footer Status
        footer = " [Esc (once: Clear, twice: Exit) | Enter: Open File | Up/Down: Select] "
        try:
        ...
        if ch == 3: # Ctrl+C -> Exit
            break
        elif ch == 27: # ESC key: 1st press clears query, 2nd press exits
            if query.strip() or results:
                query = ""
                results = []
                selected_idx = 0
                elapsed_ms = 0.0
            ...
            if query:
                query = query[:-1]
                results, elapsed_ms = engine.search(query)
                selected_idx = 0
...
def run_direct_cli_search(query):
    """Executes a single search query directly in terminal."""
    engine = SearchEngine()
    ...
    results, elapsed_ms = engine.search(query, limit=100)

    print(f"\n🔍 QSearch Results for '{query}' ({len(results)} matched in {elapsed_ms:.1f} ms):\n" + "─" * 70)
    if not results:
        print("No matches found.")
    else:
        for fname, rnum, ltext in results:
            print(f"📄 \033[1;34m{fname}\033[0m:\033[33m{rnum}\033[0m │ {ltext}")
    print("─" * 70 + "\n")
