...
class SearchEngine:
    ...
    def _init_db(self):
        ...
        # Check if existing fts_idx table uses trigram tokenizer; if not, recreate it
        try:
    ...
    def search(self, query_str, limit=300):
        """Performs multi-stage search (Exact/Substring + Fuzzy Fallback)."""
        ...
        try:
            # Stage 1: Substring / Exact Search
            if self.use_fts:
                # Trigram matches any substring (middle of words included)
                trigram_query = ' OR '.join(f'"{q}"' for q in query_str.split() if q)
                if not trigram_query:
                    trigram_query = f'"{query_str}"'
            ...
            if not results and not self.use_fts:
                # Fallback Substring LIKE %query% search
                cur.execute("""
                    SELECT file_name, row_num, line_text 
                    FROM std_idx 
            ...
            # Stage 2: Fuzzy Search Fallback (if exact/substring matching returned no results)
            if not results and len(query_str) >= 3:
                match_type = "fuzzy"
...
class BackgroundIndexer(threading.Thread):
    ...
    def sync_content_directory(self):
        """Checks for new, updated, or removed files (.csv, .txt, .log)."""
    ...
    def index_single_file(self, conn, filepath):
        """Reads and indexes a single CSV, TXT, or LOG file into SQLite."""
...
if HAS_TKINTER:
    class QSearchGUIApp:
        ...
        def _setup_ui(self):
            ...
            btn_save = ttk.Button(top_frame, text="Save Results (.txt)", command=self._save_results_to_txt)
            ...
            if selected:
                try:
                    ...
                    if 0 <= idx < len(self._current_results):
                        ...
                        if ext in (".txt", ".log") and target_path.exists():
                            try:
        ...
        def _save_results_to_txt(self):
            ...
            default_filename = f"search_results_{safe_query}.txt" if safe_query else "search_results.txt"

            file_path = filedialog.asksaveasfilename(
                defaultextension=".txt",
                filetypes=[("Text Files (*.txt)", "*.txt"), ("All Files (*.*)", "*.*")],
                initialfile=default_filename,
                title="Save Search Results as TXT File"
            )
            ...
            try:
                with open(file_path, "w", encoding="utf-8") as f:
                    ...
                    f.write("================================================================================\n")
                    f.write(" QSearch - Search Results Export\n")
                    f.write(f" Query        : {query if query else '(All)'}\n")
                    f.write(f" Generated At : {timestamp}\n")
                    f.write(f" Total Matches: {len(self._current_results)}\n")
                    f.write("================================================================================\n\n")

                    for idx, (fname, rnum, ltext, score) in enumerate(self._current_results, start=1):
                        headers = self.engine.get_file_headers(fname)
                        score_str = f" ({score}% match)" if score < 100 else ""
                        f.write(f"[{idx}] File: {fname} | Line/Row #{rnum}{score_str}\n")
                        col_lines = format_record_multiline(headers, ltext)
...
def run_interactive_cli(stdscr):
    ...
    while True:
        ...
        info_str = f" Matches: {len(results)}{type_str} | Time: {elapsed_ms:.1f}ms | Index: {fc} files ({rc:,} rows) "
        ...
        # Footer Status
        footer = " [Esc: Clear Search | Enter: Open File | c: Copy | s: Save TXT | Up/Down: Select] "
        try:
        ...
        if ch == 3: # Ctrl+C -> Exit
            ...
            if results and 0 <= selected_idx < len(results):
                ...
                try:
                    ...
                    pass
        elif ch in (ord('s'), ord('S')): # Press 's' to save overall results to TXT file
            if results:
                safe_query = "".join(c for c in query if c.isalnum() or c in (" ", "_", "-")).strip().replace(" ", "_")
                filename = f"search_results_{safe_query}.txt" if safe_query else "search_results.txt"
                out_path = CONTENT_DIR / filename
                try:
                    with open(out_path, "w", encoding="utf-8") as f:
                        timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
                        f.write("================================================================================\n")
                        f.write(" QSearch CLI - Search Results Export\n")
                        f.write(f" Query        : {query}\n")
                        f.write(f" Generated At : {timestamp}\n")
                        f.write(f" Total Matches: {len(results)}\n")
                        f.write("================================================================================\n\n")

                        for idx, (fname, rnum, ltext, score) in enumerate(results, start=1):
                            headers = engine.get_file_headers(fname)
                            score_str = f" ({score}% match)" if score < 100 else ""
                            f.write(f"[{idx}] File: {fname} | Line/Row #{rnum}{score_str}\n")
                            col_lines = format_record_multiline(headers, ltext)
