...
class SearchEngine:
    ...
    def search(self, query_str, limit=300, file_type="csv"):
        """Performs multi-stage search (Exact/Substring + Fast SQL LIKE Fallback + Bounded Fuzzy).

        file_type options:
          - 'csv': Only CSV files (.csv) [DEFAULT]
          - 'text': Only non-CSV text files (.txt, .log, .json, etc.)
        ...
        # Build SQL extension filter clause
        if file_type == "csv":
            type_clause = " AND file_name LIKE '%.csv'"
        elif file_type == "text":
            type_clause = " AND file_name NOT LIKE '%.csv'"
        else:  # 'all'
            type_clause = ""
        ...
        try:
            ...
            if self.use_fts and len(query_str) >= 3:
                try:
                    ...
                    cur.execute(f"""
                        ...
                        FROM fts_idx 
                        WHERE fts_idx MATCH ?{type_clause}
                        LIMIT ?;
                    """, (trigram_query, limit))
            ...
            if not results:
                ...
                cur.execute(f"""
                    SELECT file_name, row_num, line_text 
                    FROM {table_name} 
                    WHERE line_text LIKE ?{type_clause}
                    LIMIT ?;
                """, (f"%{query_str}%", limit))
                raw_exact = cur.fetchall()
                results = [(r[0], r[1], r[2], 100) for r in raw_exact]
...
class BackgroundIndexer(threading.Thread):
    ...
    def index_single_file(self, conn, filepath):
        ...
        # 1. Read and parse file first (before touching database)
        try:
            ...
            try:
                ...
                # Fallback to latin-1 encoding if UTF-8 parsing fails
                parsed_batch.clear()
                total_rows = 0
                headers_json = None
        ...
        # 2. Atomic Database Update (Delete old entries & Insert new entries inside transaction)
        try:
            cur = conn.cursor()
...
if HAS_TKINTER:
    class QSearchGUIApp:
        ...
        def _setup_ui(self):
            ...
            lbl_filter = ttk.Label(top_frame, text="Filter:", font=("System", 11))
            lbl_filter.pack(side="right", padx=(8, 2))
            ...
            self.filter_cb.pack(side="right", padx=(0, 4))
...
def run_interactive_cli(stdscr):
    ...
    while True:
        ...
        info_str = f" Matches: {len(results)}{type_str} | Time: {elapsed_ms:.1f}ms | Filter: {mode_label} [Tab/f to switch] | {idx_str} "
        ...
        if ch == 3: # Ctrl+C -> Exit
            ...
            elapsed_ms = 0.0
            match_type = "exact"
        elif ch in (9, ord('\t'), ord('f'), ord('F')): # Tab or 'f' key toggles file type filter
            filter_idx = (filter_idx + 1) % len(filter_modes)
            if query:
                results, elapsed_ms, match_type = engine.search(query, file_type=filter_modes[filter_idx])
                selected_idx = 0
...
def run_direct_cli_search(query, file_type="csv"):
    ...
    results, elapsed_ms, match_type = engine.search(query, limit=100, file_type=file_type)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and not sys.argv[1].startswith("-h"):
        ...
        for arg in sys.argv[1:]:
            if arg in ("--text", "-text", "-t"):
                file_type = "text"
            elif arg in ("--all", "-all", "-a"):
                file_type = "all"
            elif arg in ("--csv", "-csv", "-c"):
                file_type = "csv"
            else:
        ...
        if query_words:
            ...
            run_direct_cli_search(search_query, file_type=file_type)
            sys.exit(0)
