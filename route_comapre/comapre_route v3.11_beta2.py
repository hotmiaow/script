...
def extract_routes(source: Union[str, Path, Iterable[str]]):
    """
    Enhanced route extraction supporting both IOS and NX-OS formats with VRF support.
    """
    ...
    # Enhanced state tracking for both IOS and NX-OS
    current_vrf = "default"
    in_group = False
    group_mask = None
    variably_sub = False
    ...
    is_nxos = False  # Auto-detect NX-OS format

    # Debug flag
    DEBUG_SUBNET_GROUPS = False  # Disable for cleaner output
    ...
    def parse_nxos_route_line(line):
        """
        Parse NX-OS route line format:
        10.0.0.3/32, ubest/mbest: 1/0, attached
            *via 10.0.0.3, Eth1/27, [2/0], 00:40:52, am
        """
        line = line.strip()
        
        # Main route line (has network/prefix)
        main_route_match = re.match(r'^(\d{1,3}(?:\.\d{1,3}){3}/\d{1,2}),\s+ubest/mbest:\s+\d+/\d+(?:,\s+(.+))?', line)
        if main_route_match:
            network = main_route_match.group(1)
            flags = main_route_match.group(2) if main_route_match.group(2) else ""
            protocol = "Connected" if "attached" in flags.lower() else "Unknown"
            return "main_route", network, protocol, None, None, None
        
        # Path line (starts with *)
        path_match = re.match(r'^\s*\*via\s+(\d{1,3}(?:\.\d{1,3}){3}),\s*([^,]+),\s*\[(\d+)/(\d+)\],\s*([^,]+),\s*(.+)', line)
        if path_match:
            next_hop = path_match.group(1)
            interface = path_match.group(2).strip()
            admin_distance = int(path_match.group(3))
            metric = path_match.group(4)
            uptime = path_match.group(5).strip()
            protocol_info = path_match.group(6).strip()
            
            # Determine protocol from admin distance and protocol info
            protocol = determine_nxos_protocol(admin_distance, protocol_info)
            
            return "path", None, protocol, next_hop, uptime, interface
        
        return "unknown", None, None, None, None, None

    def determine_nxos_protocol(admin_distance, protocol_info):
        """Determine protocol from NX-OS admin distance and protocol info."""
        protocol_info = protocol_info.lower()
        
        # Direct protocol mapping
        if "ospf-" in protocol_info:
            return f"OSPF ({protocol_info})"
        elif "bgp-" in protocol_info:
            return f"BGP ({protocol_info})"
        elif "eigrp-" in protocol_info:
            return f"EIGRP ({protocol_info})"
        elif protocol_info == "static":
            return "Static"
        elif protocol_info == "am" or "attached" in protocol_info:
            return "Connected"
        elif protocol_info == "local":
            return "Local"
        
        # Fall back to admin distance
        if admin_distance in ADMINISTRATIVE_DISTANCES:
            return ADMINISTRATIVE_DISTANCES[admin_distance]
        
        return f"Unknown (AD:{admin_distance})"

    i = 0
    current_main_route = None
    
    while i < len(lines):
        raw = lines[i]
        line = raw.strip()

        # Skip empty lines and some banners
        if not line or line.startswith(("Codes:", "Gateway of last resort", "'*' denotes", "'**' denotes", "'[x/y]' denotes")):
            i += 1
            continue

        # Check for VRF header
        is_vrf, vrf_name = detect_vrf_header(line)
        if is_vrf:
            current_vrf = vrf_name
            log_debug(f"Switched to VRF: {current_vrf}")
            i += 1
            continue

        # Auto-detect NX-OS format
        if "ubest/mbest:" in line or "*via" in line:
            is_nxos = True

        if is_nxos:
            # Handle NX-OS format
            route_type, network, protocol, next_hop, uptime, interface = parse_nxos_route_line(line)
            
            if route_type == "main_route":
                current_main_route = network
                if protocol == "Connected":
                    # For connected routes, we might have the info on the same line
                    vrf_network = f"{network}@{current_vrf}" if current_vrf != "default" else network
                    routes[vrf_network].append((protocol, "Connected", uptime or "N/A"))
                    
            elif route_type == "path" and current_main_route:
                vrf_network = f"{current_main_route}@{current_vrf}" if current_vrf != "default" else current_main_route
                routes[vrf_network].append((protocol, next_hop, uptime))
                
        else:
            # Handle traditional IOS format (existing logic)
            ...
            is_group_detected, base_network, cidr_mask, is_variably, expected_count = detect_subnet_group(raw)

            if is_group_detected:
            ...
            if proto_match:
                ...
                # Add VRF prefix if not default
                if current_vrf != "default":
                    norm_subnet = f"{norm_subnet}@{current_vrf}"
...
def detect_vrf_header(line):
    ...
    """
    # Pattern: IP Route Table for VRF "vrf_name"
    vrf_match = re.search(r'IP\s+Route\s+Table\s+for\s+VRF\s+"([^"]+)"', line, re.IGNORECASE)
    if vrf_match:
        return True, vrf_match.group(1)
    return False, None
...
def compare_device_outputs():
    ...
    for device_name in sorted(all_devices):
        ...
        try:
            ...
            before_block = extract_command_block(str(before_file), "show ip route")
            
            print(f"  Extracting routes from {after_file.name}...")
            after_block  = extract_command_block(str(after_file),  "show ip route")

            routes1 = extract_routes(before_block)   # pass a list of lines
            routes2 = extract_routes(after_block)    # idem
    ...
    while i < len(lines):
        ...
        if not inside:
            ...
            if ln.strip().lower() == wanted_cmd:
                inside = True          # start grabbing
                i += 2                 # skip the dashed underline
                continue
        else:
            # a new header starts → stop
            if re.match(r"^\s*[a-z].*$", ln) and i + 1 < len(lines) \
