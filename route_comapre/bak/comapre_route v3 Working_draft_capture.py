...
def capture_device_outputs(mode, input_csv, username=None, password=None):
    """
    Capture outputs from all devices.

    • Mode 1 → folder “before”   • Mode 2 → folder “after”
    • Each *single-command* log lives in <folder>/commands/
      –- except for “terminal length/page” housekeeping commands, whose
        outputs are kept only in the per-device combined file.
    • After the last command per device, a combined file is written in the
      parent folder (<before> or <after>).
    """
    import datetime, re, textwrap

    # Commands to SKIP for individual files (case-insensitive, any spacing)
    _IGNORE_PATTERNS = [
    ...
    def _skip_individual(cmd: str) -> bool:
        """Return True if cmd should NOT get its own file."""
    ...
    # One credential prompt for the whole run
    if not username:
    ...
    for idx_dev, dev in enumerate(devices, 1):
        ...
        cmd_lines   = re.split(r"\r\n|\n|\r", raw_field) if raw_field else []
        commands    = [c.strip() for c in cmd_lines if c.strip()] or ["show ip route"]

        print(f"[{idx_dev}/{len(devices)}] {device_name}: {len(commands)} command(s)")

        combined_chunks = []   # build the per-device combined file
        for idx_cmd, command in enumerate(commands, 1):
            print(f"   → ({idx_cmd}/{len(commands)}) {command}")
            ...
            header = f"\n{command}\n" + "-" * len(command) + "\n"
            combined_chunks.append(header + (output or "") + "\n")

            if good:
                print(f"     ✓ completed OK  ({elapsed:.1f}s)")
                ok += 1
            else:
            ...
            # Only create an individual file if command is not in ignore list
            if not _skip_individual(command):
                save_device_output(device_name, command, output, folder_path)
            else:
                print("     ↳ individual file suppressed (housekeeping command)")

        # ----- write the combined file for this device --------------------
        if combined_chunks:
            ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            combined_file = folder_path / f"{device_name}_combined_{ts}.txt"
            with open(combined_file, "w", encoding="utf-8") as fh:
                divider = "\n" + "=" * 80 + "\n"
                fh.write(divider.join(combined_chunks))
