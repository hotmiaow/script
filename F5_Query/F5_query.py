...
def parse_gtm_member_name(gtm_member_name, gtm_servers=None):
    """
    Parses a GTM pool member string into (server_name, vs_name).
    Handles formats:
      - '/Common/server1:vs_app1' -> ('/Common/server1', 'vs_app1')
      - '/Common/server1:/INT/1.1.1.1%1:443' -> ('/Common/server1', '/INT/1.1.1.1%1:443')
      - '/INT/1.1.1.1%1:443' -> (None, '/INT/1.1.1.1%1:443')
      - '1.1.1.1:443' -> (None, '1.1.1.1:443')
    """
    if not gtm_member_name:
        return None, ""

    clean_mem = gtm_member_name.strip()
    if ':' not in clean_mem:
        return None, clean_mem

    parts = clean_mem.split(':')
    first_part = parts[0].strip()

    # Check if first_part matches a known GTM server name
    server_key = None
    if gtm_servers:
        if first_part in gtm_servers:
            server_key = first_part
        elif normalize_name(first_part) in gtm_servers:
            server_key = normalize_name(first_part)
        else:
            for s_k in gtm_servers.keys():
                if normalize_name(s_k) == normalize_name(first_part):
                    server_key = s_k
                    break

    if server_key:
        vs_name = clean_mem[len(first_part) + 1:].strip()
        return server_key, vs_name
    else:
        # Not a server prefix; entire string is vs_name (e.g. /INT/1.1.1.1%1:443)
        return None, clean_mem


def find_ltm_virtual(ltm_virtuals, vs_name, gtm_servers=None, server_name=None):
    """
    ...
    clean_vs_ip = clean_target_address(vs_name)

    # 2. Normalized name lookup (partition & route domain & port agnostic)
    for k, v in ltm_virtuals.items():
        k_norm = normalize_name(k)
        k_ip = clean_target_address(k)
        if k == vs_name or k_norm == clean_vs or (clean_vs_ip and k_ip == clean_vs_ip):
            return k, v

    # 3. Collect candidate target IPs
    target_ips = set()
    if clean_vs_ip:
        target_ips.add(clean_vs_ip)

    if gtm_servers and server_name:
        srv_obj = gtm_servers.get(server_name) or gtm_servers.get(normalize_name(server_name))
        if isinstance(srv_obj, dict):
            # Check server addresses
            srv_addrs = srv_obj.get('addresses') or srv_obj.get('address')
            if srv_addrs:
                addrs_dict = extract_name_dict(srv_addrs)
                for addr_k in addrs_dict.keys():
                    c_addr = clean_target_address(addr_k)
                    if c_addr: target_ips.add(c_addr)

            # Check server virtual-servers block
            vservers = srv_obj.get('virtual-servers') or srv_obj.get('virtual-server')
            if vservers:
                vs_dict = extract_name_dict(vservers)
                v_obj = vs_dict.get(vs_name) or vs_dict.get(normalize_name(vs_name))
                if isinstance(v_obj, dict):
                    dest = v_obj.get('destination') or v_obj.get('address')
                    if dest:
                        if isinstance(dest, dict):
                            dest_str = list(dest.keys())[0] if dest else ''
                        else:
                            dest_str = str(dest)
                        c_dest = clean_target_address(dest_str)
                        if c_dest: target_ips.add(c_dest)

    # 4. Destination IP / target address lookup in ltm_virtuals
    if target_ips:
        for k, v in ltm_virtuals.items():
            if isinstance(v, dict):
                dest_raw = v.get('destination') or v.get('address')
                if isinstance(dest_raw, dict):
                    dest_str = list(dest_raw.keys())[0] if dest_raw else ''
                else:
                    dest_str = str(dest_raw or '')
                dest_ip = clean_target_address(dest_str)
                if dest_ip and dest_ip in target_ips:
                    return k, v
...
class F5Parser:
    def __init__(self):
        self.gtm_wideips = {}
        self.gtm_wideips_all = []      # List of all WideIP entries across files: {'fqdn', 'item', 'source_file', 'hostname'}
        self.gtm_pools = {}
        self.gtm_servers = {}
        self.ltm_virtuals = {}
        self.ltm_pools = {}
        ...
        self.object_sources = {}       # object_name -> list of {'source_file', 'hostname'}
    ...
    def parse_text(self, text, source_file=None, hostname=None):
        ...
        for key, value in root.items():
            ...
            for item in items:
                ...
                if obj_name:
                    if source_file or hostname:
                        src_entry = {'source_file': source_file or '-', 'hostname': hostname or '-'}
                        if obj_name not in self.object_sources:
                            self.object_sources[obj_name] = []
                        if src_entry not in self.object_sources[obj_name]:
                            self.object_sources[obj_name].append(src_entry)
...
def load_configs(config_dir, progress_callback=None):
    parser = F5Parser()
...
def walk_relationships(parser, query_fqdn=None, query_ip=None, query_file=None, query_host=None):
    ...
    # Pre-calculate GTM redundancy mappings across all WideIP declarations
    wip_entries = parser.gtm_wideips_all if (hasattr(parser, 'gtm_wideips_all') and parser.gtm_wideips_all) else [
        {'fqdn': k, 'item': v, 'source_file': parser.object_source_file.get(k, '-'), 'hostname': parser.object_hostname.get(k, '-')}
        for k, v in parser.gtm_wideips.items()
    ]

    vs_gtm_map = {}    # vs_key -> list of {'gtm_file': ..., 'gtm_host': ..., 'fqdn': ...}
    fqdn_gtm_map = {}  # fqdn -> list of {'gtm_file': ..., 'gtm_host': ...}

    for entry in wip_entries:
        fqdn = entry['fqdn']
        wip_src = entry['source_file']
        wip_host = entry['hostname']
        if fqdn not in fqdn_gtm_map:
            fqdn_gtm_map[fqdn] = []
        fqdn_gtm_map[fqdn].append({'gtm_file': wip_src, 'gtm_host': wip_host})

        wip_pools_raw = entry['item'].get('pools') or entry['item'].get('pool') if isinstance(entry['item'], dict) else entry['item']
        wip_pools = extract_name_dict(wip_pools_raw)
        for gtm_pool_name in wip_pools.keys():
            ...
            for gtm_member_name in pool_members.keys():
                server_name, vs_name = parse_gtm_member_name(gtm_member_name, parser.gtm_servers)
                vs_key, _ = find_ltm_virtual(parser.ltm_virtuals, vs_name, parser.gtm_servers, server_name)
                target_key = vs_key if vs_key else vs_name
                if target_key not in vs_gtm_map:
                    vs_gtm_map[target_key] = []
                vs_gtm_map[target_key].append({'gtm_file': wip_src, 'gtm_host': wip_host, 'fqdn': fqdn})

    for entry in wip_entries:
        fqdn = entry['fqdn']
        wideip_data = entry['item']
        wip_src = entry['source_file']
        wip_host = entry['hostname']
        ...
        for gtm_pool_name, gtm_pool_data in wip_pools.items():
            ...
            for gtm_member_name, gtm_member_data in pool_members.items():
                ...
                vs_key, ltm_vs_obj = find_ltm_virtual(parser.ltm_virtuals, vs_name, parser.gtm_servers, server_name)
                if vs_key:
                    seen_ltm_vs.add(vs_key)
                    seen_ltm_vs.add(normalize_name(vs_key))
                
                target_key = vs_key if vs_key else vs_name
                vs_refs = vs_gtm_map.get(target_key, [])
                unique_hosts = sorted(set(r['gtm_host'] for r in vs_refs if r['gtm_host'] != '-'))
                unique_files = sorted(set(r['gtm_file'] for r in vs_refs if r['gtm_file'] != '-'))

                if len(vs_refs) > 1:
                    if len(unique_hosts) > 1:
                        red_status = f"🔁 Redundant ({len(vs_refs)} GTM refs across hosts: {', '.join(unique_hosts)})"
                    elif len(unique_files) > 1:
                        red_status = f"🔁 Redundant ({len(vs_refs)} GTM refs across files: {', '.join(unique_files)})"
                    else:
                        red_status = f"🔁 Redundant ({len(vs_refs)} GTM refs)"
                else:
                    red_status = "Unique (Single GTM Ref)"
...
def run_gui():
    ...
    class F5QueryApp(tk.Tk):
        ...
        def _create_widgets(self):
            ...
            # Row 4: Progress Bar & Activity Details
            self.progress_frame = ttk.Frame(ctrl_frame)
            self.progress_frame.grid(row=4, column=0, columnspan=5, padx=5, pady=4, sticky=tk.EW)

            self.progress_bar = ttk.Progressbar(self.progress_frame, mode='determinate')
            self.progress_bar.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(5, 10))

            self.progress_detail_var = tk.StringVar(value="")
            self.progress_detail_lbl = ttk.Label(self.progress_frame, textvariable=self.progress_detail_var, font=("TkDefaultFont", 9, "bold"), foreground="#0275d8")
            self.progress_detail_lbl.pack(side=tk.LEFT, padx=5)

            self.action_buttons = [load_btn, browse_dir_btn, browse_file_btn, filter_btn, clear_btn, ping_btn, export_btn]
        ...
        def _set_ui_busy(self, busy=True, status_text="", progress_val=0, detail_text=""):
            ...
            if busy:
                self.progress_bar.config(mode='determinate', value=progress_val)
                self.progress_detail_var.set(detail_text or status_text)
        ...
        def _update_progress_ui(self, current, total, text):
            pct = (current / total) * 100 if total > 0 else 0
            self.progress_bar.config(mode='determinate', value=pct)
            self.progress_detail_var.set(f"Reading ({current}/{total}): {text}")
            self.status_var.set(f"⏳ Reading file {current} of {total}: {text}")
        ...
        def load_configs_gui(self):
            ...
            self._set_ui_busy(True, f"Scanning files in '{os.path.basename(config_dir)}'...", progress_val=5)

            def _worker():
                def _cb(curr, total, fname):
                    self.after(0, lambda c=curr, t=total, f=fname: self._update_progress_ui(c, t, f))

                parser = load_configs(config_dir, progress_callback=_cb)
                self.after(0, lambda: self._on_load_configs_done(parser, config_dir))

            threading.Thread(target=_worker, daemon=True).start()

        def _on_load_configs_done(self, parser, config_dir):
            self.f5_parser = parser
        ...
        def apply_filter(self):
            ...
            self._set_ui_busy(True, "Querying and mapping F5 relationships...", progress_val=50, detail_text="Mapping relationships across configurations...")

            def _worker():
                results = walk_relationships(
                    self.f5_parser, query_fqdn=fqdn, query_ip=ip, query_file=q_file, query_host=q_host
                )
                self.after(0, lambda: self._on_filter_done(results))
                ...
                for gpool, gmems in gpools.items():
                    ...
                    for gmem, rows in gmems.items():
                        ...
                        for r in rows:
                            ...
                            is_redundant = 'Redundant' in red_str
                            ...
                            if is_pending:
                                ...
                                red_tag = " [🔁 Redundant GTM Entry]" if is_redundant else ""
                                vs_label = f"LTM VS: {r['ltm_vs']}{red_tag}"
                                ltm_file = r.get('ltm_source_file', '-')
                                ltm_host = r.get('ltm_hostname', '-')
        ...
        def _setup_detail_tab(self):
            ...
            col_widths = {
                "LTM Status": 160, "Redundancy": 220, "GTM File": 110, "GTM Host": 140,
            ...
            self.detail_tree.tag_configure("redundant", foreground="#0055b8", font=("TkDefaultFont", 9, "bold"))
        ...
        def populate_detail_view(self):
            ...
            for r in self.current_results:
                ...
                if 'Pending' in ltm_status or 'Unavailable' in ltm_status:
                    tag = "pending"
                elif 'Redundant' in red_str:
                    tag = "redundant"
                else:
                    tag = "ok"
