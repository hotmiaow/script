...
def load_configs(config_dir):
    ...
    for filepath in files:
        with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
            content = f.read()
            has_ltm = bool(re.search(r'\bltm\s+', content, re.IGNORECASE))
            has_gtm = bool(re.search(r'\bgtm\s+', content, re.IGNORECASE))
            if has_ltm:
...
def export_csv(results, filepath, mode='both'):
    ...
    headers = ["FQDN", "GTM Pool", "GTM Member", "LTM VS", "VS Dest", "Description", "SNAT", "iRules", "Profiles", "LTM Pool", "LTM Member", "Member IP"]
...
def print_table(results):
    ...
    headers = ["FQDN", "GTM Pool", "GTM Member", "LTM VS", "VS Dest", "Description", "SNAT", "iRules", "LTM Pool", "LTM Member", "Member IP"]
    ...
    for r in results:
        ...
        widths[5] = max(widths[5], len(r.get('description', '-')))
        widths[6] = max(widths[6], len(r.get('snat', '-')))
        irules_str = ", ".join(r.get('irules', [])) if r.get('irules') else "-"
        widths[7] = max(widths[7], len(irules_str))
        widths[8] = max(widths[8], len(r['ltm_pool']))
        ...
        widths[9] = max(widths[9], len(members_str))
        widths[10] = max(widths[10], len(ips_str))
...
def run_gui():
    ...
    class F5QueryApp(tk.Tk):
        ...
        def _create_widgets(self):
            ...
            ttk.Label(ctrl_frame, text="Filter FQDN:").grid(row=2, column=0, padx=5, pady=5, sticky=tk.W)
            self.fqdn_var = tk.StringVar()
            fqdn_entry = ttk.Entry(ctrl_frame, textvariable=self.fqdn_var, width=22)
            fqdn_entry.grid(row=2, column=1, padx=5, pady=5, sticky=tk.W)
            fqdn_entry.bind("<KeyRelease>", self.on_filter_key)
            fqdn_entry.bind("<Return>", lambda e: self.apply_filter())

            ttk.Label(ctrl_frame, text="Filter IP:").grid(row=2, column=2, padx=5, pady=5, sticky=tk.E)
            self.ip_var = tk.StringVar()
            ip_entry = ttk.Entry(ctrl_frame, textvariable=self.ip_var, width=18)
            ip_entry.grid(row=2, column=3, padx=5, pady=5, sticky=tk.W)
            ...
            btn_subframe.grid(row=2, column=4, padx=5, pady=5, sticky=tk.E)
        ...
        def _setup_context_menus(self):
            ...
            for tree in [self.visual_tree, self.detail_tree, self.summary_tree]:
