...
def extract_domains_from_pac(pac_file, skip_subnets=False):
    ...
    try:
        ...
        # Matches: dnsDomainIs(host, ".domain.com") or dnsDomainIs(host, 'domain.com')
        pattern_dnsDomainIs = re.compile(r"""dnsDomainIs\s*\(\s*[^,]+,\s*['"]\.?([a-zA-Z0-9-]+\.[a-zA-Z0-9.-]+)['"]\s*\)""", re.IGNORECASE)
        
        # Pattern 3: Extract from shExpMatch with domain patterns like "*.example.com" or "*example.com*"
        # Matches: shExpMatch(host, "*.domain.com") or shExpMatch(url, "*domain.com*")
...
def create_no_dns_pac(original_pac_file):
    ...
    try:
        ...
        # Add mock DNS functions at the beginning of the PAC file
        dns_mocks = """
...
def workflow_setup_baseline():
    ...
    if not run_logic_test("test_logic.csv", OLD_PAC_DIR, "before_logic_result.csv", skip_dns=True):
...
def workflow_test_and_compare():
    ...
    if not run_logic_test("test_logic.csv", UPDATED_PAC_DIR, "after_logic_result.csv", skip_dns=True):
...
def workflow_test_and_compare_logic_only():
    ...
    - Download updated PAC files to updated_PAC/
    - Run AFTER logic test
    - Compare with baseline logic results
    """
    print("\n" + "="*60)
...
│     • Download PAC files, generate test CSVs               │
│     • Run logic & reachability tests                       │
│                                                             │
│  2) Test Updated PAC & Compare (Full)                      │
│     • Download updated PAC files                           │
│     • Run tests & generate comparison reports              │
│                                                             │
│  3) Setup Baseline (Logic Only - Fast)                     │
│     • Download PAC files, generate logic test CSV          │
│     • Run logic test only (no connectivity test)           │
│                                                             │
│  4) Test Updated PAC & Compare (Logic Only - Fast)         │
│     • Download updated PAC files                           │
│     • Run logic test & generate comparison report          │
│                                                             │
└─────────────────────────────────────────────────────────────┘
...
│  7) Generate test_reachability.csv from old_PAC/           │
│                                                             │
│  8) Run logic test on old_PAC/                             │
│  9) Run logic test on updated_PAC/                         │
│                                                             │
│ 10) Run reachability test (downloads live PAC files)       │
│ 11) Run reachability test on old_PAC/ (local files)        │
│ 12) Run reachability test on updated_PAC/ (local files)    │
│                                                             │
└─────────────────────────────────────────────────────────────┘
...
""")
        ...
        if choice == "1":
            ...
            if not os.path.exists("test_logic.csv"):
                print("\n❌ test_logic.csv not found. Generate it first (option 6).")
            else:
                run_logic_test("test_logic.csv", OLD_PAC_DIR, "manual_old_logic_result.csv", skip_dns=True)
            
        elif choice == "9":
            if not os.path.exists("test_logic.csv"):
                print("\n❌ test_logic.csv not found. Generate it first (option 6).")
            else:
                run_logic_test("test_logic.csv", UPDATED_PAC_DIR, "manual_updated_logic_result.csv", skip_dns=True)
            
        elif choice == "10":
            if not os.path.exists("test_reachability.csv"):
                print("\n❌ test_reachability.csv not found. Generate it first (option 7).")
            else:
