...
def timeout_handler(signum, frame):
    raise TimeoutError("PAC evaluation timeout")
...
def create_no_dns_pac(original_pac_file):
    ...
    try:
        ...
        dns_mocks = """
// Mock DNS functions to avoid slow lookups
function dnsResolve(host) {
    return "127.0.0.1";  // Return dummy IP
}
...
function myIpAddress() {
    return "192.168.1.100";  // Return dummy IP
}
...
"""
        ...
        # Combine mocks with original content
...
def evaluate_pac(pac_file, url, skip_dns=True):
    ...
    try:
        # Create DNS-free version if requested
        if skip_dns:
