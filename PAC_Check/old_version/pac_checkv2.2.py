...
def run_reachability_test(output_file):
    """Test actual connectivity based on test_reachability.csv."""
    ...
    print("\n[Step] Running reachability test...")
    with open(output_file, "w", newline="") as out:
        writer = csv.writer(out)
...
def compare_logic_results(before, after, output):
    ...
    print(f"  ✓ Comparison completed → {output}")
    print(f"  ✓ Changes detected: {changes_count}")
    return True


def compare_reachability_results(before, after, output):
    ...
    print(f"  ✓ Reachability comparison completed → {output}")
    print(f"  ✓ Changes detected: {changes_count}")
    return True
...
def workflow_test_and_compare():
    ...
    print("    - pac_logic_comparison_report.csv        (logic change report)")
    print("    - pac_reachability_comparison_report.csv (reachability change report)")
    print("\n  Review comparison reports for changes:")
    print("    • pac_logic_comparison_report.csv - PAC routing logic changes")
    print("    • pac_reachability_comparison_report.csv - Connectivity impact")
    print("="*60 + "\n")
...
""")
        ...
        if choice == "1":
            ...
            run_reachability_test("reachability_result.csv")
