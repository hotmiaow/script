...
# Global lock for pacparser (not thread-safe on any platform)
_pacparser_lock = threading.Lock()

PAC_LIST = "all_pac.csv"
BACKUP_DIR = "backup"
OLD_PAC_DIR = "old_PAC"
UPDATED_PAC_DIR = "updated_PAC"
TEST_CASES_DIR = "test_cases"      # New: folder for test CSV files
OUTPUT_DIR = "output"              # New: folder for results
MAX_BACKUPS = 10
TIMEOUT = 5
...
def setup_folders():
    ...
    folders = [
        ...
        UPDATED_PAC_DIR,
        TEST_CASES_DIR,
        os.path.join(OUTPUT_DIR, "logic"),
        os.path.join(OUTPUT_DIR, "reachability"),
...
def check_connectivity_windows(host, port):
    """Test connectivity on Windows using Test-NetConnection."""
    cmd = [
        "powershell", "-Command",
...
def is_subnet_or_range(domain):
    """Check if the domain is actually a subnet/IP range."""
...
def generate_test_logic_csv_from_folder(pac_folder, output_file):
    ...
    output_path = os.path.join(TEST_CASES_DIR, os.path.basename(output_file))
    
    print(f"\n[Step] Generating {os.path.basename(output_file)} from {pac_folder}/...")
    ...
    success, final_file = safe_write_file(output_path, write_csv)
    if success:
...
def evaluate_pac(pac_file, url, skip_dns=True):
    """Cross-platform thread-safe PAC evaluation using a global lock."""
    actual_pac_file = pac_file
    
    with _pacparser_lock:
        try:
            if skip_dns:
            ...
            if skip_dns and actual_pac_file != pac_file and os.path.exists(actual_pac_file):
                try:
...
def run_logic_test(test_csv, pac_folder, output_file, skip_dns=True):
    ...
    if not os.path.isabs(test_csv) and not os.path.exists(test_csv):
        test_csv = os.path.join(TEST_CASES_DIR, test_csv)
    ...
    output_path = os.path.join(OUTPUT_DIR, "logic", os.path.basename(output_file))
...
def run_reachability_test_from_folder(pac_folder, test_csv, output_file, max_workers=20):
    ...
    output_path = os.path.join(OUTPUT_DIR, "reachability", os.path.basename(output_file))
    ...
    with open(test_csv, encoding="utf-8-sig") as f:
        reader = csv.DictReader(f)
        for row in reader:
            pac_file = os.path.join(pac_folder, f"{row['pac_name']}.pac")
    ...
    def test_single_url(test_case):
        pac_name = test_case['pac_name']
        pac_path = test_case['pac_path']
        test_url = test_case['test_url']
        pac_file = test_case['pac_file']
        
        try:
            decision = evaluate_pac(pac_file, test_url, skip_dns=True)
            reachable = check_connectivity(decision, test_url)
            return [pac_name, pac_path, test_url, decision, reachable]
        except Exception as e:
            return [pac_name, pac_path, test_url, f"ERROR: {str(e)}", "NO"]
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_test = {executor.submit(test_single_url, tc): tc for tc in test_cases}
        ...
        for future in as_completed(future_to_test):
            try:
                ...
                if completed % 10 == 0 or completed == total:
                    print(f"  Progress: {completed}/{total} ({100*completed//total}%) - Errors: {errors}")
                
                pac_name, _, test_url, decision, reachable = result
                if "ERROR" in decision:
                    print(f"    ✗ {pac_name}: {test_url} -> {decision}")
                else:
                    status_icon = "✓" if reachable == "YES" else "✗"
                    print(f"    {status_icon} {pac_name}: {test_url} -> {decision} [{reachable}]")
    ...
    success, final_file = safe_write_file(output_path, write_csv)
    if success:
        print(f"\n  ✓ Reachability test completed → {final_file}")
        print(f"  Summary: {total} tests, {errors} errors, {total-errors} successful")
    return success


def compare_logic_results(before, after, output_full, output_summary):
    """Compare BEFORE and AFTER PAC logic test results."""
    before_path = before if os.path.isabs(before) else os.path.join(OUTPUT_DIR, "logic", before)
    after_path = after if os.path.isabs(after) else os.path.join(OUTPUT_DIR, "logic", after)
    
    if not os.path.exists(before_path) or not os.path.exists(after_path):
        print("Error: Missing before/after result files.")
        return False

    full_path = os.path.join(OUTPUT_DIR, "comparison", os.path.basename(output_full))
    summary_path = os.path.join(OUTPUT_DIR, "comparison", os.path.basename(output_summary))
    ...
    success_full, final_full = safe_write_file(full_path, write_full)
    ...
    success_summary, final_summary = safe_write_file(summary_path, write_summary)
...
def compare_reachability_results(before, after, output_full, output_summary):
    ...
    before_path = before if os.path.isabs(before) else os.path.join(OUTPUT_DIR, "reachability", before)
    after_path = after if os.path.isabs(after) else os.path.join(OUTPUT_DIR, "reachability", after)
...
# ---------- WORKFLOW FUNCTIONS ----------
def workflow_setup_baseline(max_workers=20):
    ...
    if not run_reachability_test_from_folder(OLD_PAC_DIR, "old_test_reachability.csv", "before_reachability_result.csv", max_workers=max_workers):
    ...
    print(f"    - {TEST_CASES_DIR}/old_test_logic.csv              (logic test cases)")
    print(f"    - {TEST_CASES_DIR}/old_test_reachability.csv       (reachability test cases)")
    print(f"    - {OUTPUT_DIR}/logic/before_logic_result.csv         (baseline logic results)")
    print(f"    - {OUTPUT_DIR}/reachability/before_reachability_result.csv  (baseline reachability)")
    print("\n  Next step: Make your PAC file changes, then run Workflow 2")
    print("="*60 + "\n")


def workflow_test_and_compare(max_workers=20):
    """Workflow 2: Test updated PAC files and compare (FULL - with reachability)"""
    print("\n" + "="*60)
    print("  WORKFLOW 2: Test Updated PAC Files & Compare (Full)")
    print("="*60)
    
    before_logic = os.path.join(OUTPUT_DIR, "logic", "before_logic_result.csv")
    before_reach = os.path.join(OUTPUT_DIR, "reachability", "before_reachability_result.csv")
    
    if not os.path.exists(before_logic):
        print("\n❌ Baseline logic results not found!")
        print("  Please run Workflow 1 (Setup Baseline) first.")
        return
    
    if not os.path.exists(before_reach):
    ...
    if not run_reachability_test_from_folder(UPDATED_PAC_DIR, "updated_test_reachability.csv", "after_reachability_result.csv", max_workers=max_workers):
    ...
    print(f"    - {TEST_CASES_DIR}/updated_test_*.csv                    (test cases)")
    print(f"    - {OUTPUT_DIR}/logic/after_logic_result.csv                (logic results)")
    print(f"    - {OUTPUT_DIR}/reachability/after_reachability_result.csv (reachability results)")
    print(f"    - {OUTPUT_DIR}/comparison/pac_*_comparison_*.csv           (comparison reports)")
    print("\n  📊 Review summary reports in output/comparison/ for quick impact assessment")
    print("="*60 + "\n")
...
def workflow_test_and_compare_logic_only():
    ...
    print("\n  📁 Folder Structure:")
    print(f"    - {UPDATED_PAC_DIR}/                          (updated PAC files)")
    print(f"    - {TEST_CASES_DIR}/updated_test_logic.csv                (logic test cases)")
    print(f"    - {OUTPUT_DIR}/logic/after_logic_result.csv                (logic results)")
    print(f"    - {OUTPUT_DIR}/comparison/pac_logic_comparison_*.csv       (comparison reports)")
    print("\n  ⚡ Fast execution: Reachability tests skipped")
    print("  📊 Review summary report for routing logic changes")
    print("="*60 + "\n")


def test_dns_patterns():
    """Test DNS pattern matching for debugging."""
    test_domains = [
        "apple.com",
        "www.apple.com",
        "test.apple.com",
    ...
    print("\n" + "="*60)
    print("  DNS PATTERN TESTING")
    print("="*60)
    print(f"\n  Configuration:")
    print(f"    • {len(CUSTOM_DNS_MAPPINGS)} exact mappings")
    print(f"    • {len(DNS_PATTERN_MAPPINGS)} pattern rules")
    print(f"    • Default IP: {DEFAULT_PUBLIC_IP}")
    print("\n" + "-"*60)
    ...
    for domain in test_domains:
        ...
        if domain.lower() in CUSTOM_DNS_MAPPINGS:
            resolved_ip = CUSTOM_DNS_MAPPINGS[domain.lower()]
            match_type = "exact"
        else:
            for python_pattern, ip, js_pattern in DNS_PATTERN_MAPPINGS:
                if re.search(python_pattern, domain, re.IGNORECASE):
                    resolved_ip = ip
                    match_type = f"pattern: {python_pattern}"
                    break
        
        status = "✓" if resolved_ip != DEFAULT_PUBLIC_IP else "·"
        print(f"  {status} {domain:30} → {resolved_ip:15} ({match_type})")
...
# ---------- MENU ----------
def menu():
    """Main menu with simplified layout."""
    while True:
        print("""
╔══════════════════════════════════════════════════════════╗
...
┌─ QUICK START ──────────────────────────────────────────┐
│  [1] Full Test     - Baseline → Logic + Reachability   │
│  [2] Full Compare  - Test changes with full validation │
│  [3] Quick Test    - Baseline → Logic only (faster)    │
│  [4] Quick Compare - Test changes, logic only          │
└────────────────────────────────────────────────────────┘

┌─ UTILITIES ────────────────────────────────────────────┐
│  [5] Backup PAC files                                   │
│  [6] Reload DNS config (dns_config.json)               │
│  [7] Test DNS patterns (debug)                         │
└────────────────────────────────────────────────────────┘
...
""")
        choice = input("Select [1-8 or 0]: ").strip()
        ...
        if choice == "1":
            ...
            print("\n❌ Invalid selection. Please choose 0-8.\n")
...
┌─ GENERATE TEST FILES ──────────────────────────────────┐
│  [1] Generate test_logic.csv from old_PAC/             │
│  [2] Generate test_logic.csv from updated_PAC/         │
│  [3] Generate test_reachability.csv from old_PAC/      │
│  [4] Generate test_reachability.csv from updated_PAC/  │
└────────────────────────────────────────────────────────┘

┌─ RUN TESTS ────────────────────────────────────────────┐
│  [5] Run logic test on old_PAC/                        │
│  [6] Run logic test on updated_PAC/                    │
│  [7] Run reachability test on old_PAC/                 │
│  [8] Run reachability test on updated_PAC/             │
└────────────────────────────────────────────────────────┘
...
""")
        ...
        if choice == "1":
            ...
            test_csv_path = os.path.join(TEST_CASES_DIR, "old_test_logic.csv")
            if not os.path.exists(test_csv_path):
                print("\n❌ old_test_logic.csv not found. Generate it first (option 1).\n")
            else:
                run_logic_test("old_test_logic.csv", OLD_PAC_DIR, "manual_old_logic_result.csv", skip_dns=True)
        elif choice == "6":
            test_csv_path = os.path.join(TEST_CASES_DIR, "updated_test_logic.csv")
            if not os.path.exists(test_csv_path):
                print("\n❌ updated_test_logic.csv not found. Generate it first (option 2).\n")
            else:
                run_logic_test("updated_test_logic.csv", UPDATED_PAC_DIR, "manual_updated_logic_result.csv", skip_dns=True)
        elif choice == "7":
            test_csv_path = os.path.join(TEST_CASES_DIR, "old_test_reachability.csv")
            if not os.path.exists(test_csv_path):
                print("\n❌ old_test_reachability.csv not found. Generate it first (option 3).\n")
            else:
                run_reachability_test_from_folder(OLD_PAC_DIR, "old_test_reachability.csv", "manual_old_reachability_result.csv")
        elif choice == "8":
            test_csv_path = os.path.join(TEST_CASES_DIR, "updated_test_reachability.csv")
            if not os.path.exists(test_csv_path):
                print("\n❌ updated_test_reachability.csv not found. Generate it first (option 4).\n")
            else:
