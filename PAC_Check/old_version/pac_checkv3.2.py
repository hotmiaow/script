...
def evaluate_pac(pac_file, url, skip_dns=True):
    ...
    with _pacparser_lock:  # Serializes pacparser access across all platforms
        try:
            if skip_dns:
...
def run_reachability_test_from_folder(pac_folder, test_csv, output_file, max_workers=5):
    """
    Test actual connectivity using PAC files from local folder.
    Cross-platform parallel processing (Windows, Linux, macOS).
    
    Args:
        pac_folder: Folder containing PAC files
        test_csv: CSV file with test cases
    ...
    def test_single_url(test_case):
        ...
        try:
            # PAC evaluation (serialized with lock, works on all platforms)
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_test):
            try:
                ...
                # Progress indicator every 10 tests or at completion
                if completed % 10 == 0 or completed == total:
                    print(f"  Progress: {completed}/{total} ({100*completed//total}%) - Errors: {errors}")
                
                # Show individual result (compact format)
