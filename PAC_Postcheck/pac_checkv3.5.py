...
def run_sanity_check_on_folder(pac_folder, output_csv):
    """
    Run the PAC logic sanity check to catch specific rules placed under wildcard rules.
    """
    if not os.path.exists(pac_folder) or not os.path.exists(PAC_LIST):
        print(f"Error: {pac_folder} or {PAC_LIST} not found.")
        return False
    ...
    def write_csv(filename):
        with open(filename, "w", newline="") as out:
            writer = csv.writer(out)
            writer.writerow(["pac_name", "status", "message"])
            writer.writerows(results)

    success, final_file = safe_write_file(output_csv, write_csv)
    if success:
        print(f"  ✓ Sanity check completed → {final_file}")
    return success
...
def compare_sanity_results(before, after, output_summary):
    ...
    def write_csv(filename):
        with open(filename, "w", newline="") as out:
            writer = csv.writer(out)
            writer.writerow(["pac_name", "change_type", "message"])
            writer.writerows(results)
            
    success, final_file = safe_write_file(output_summary, write_csv)
    if success:
        print(f"  ✓ Sanity comparison completed → {final_file}")
...
def run_header_test_from_folder(pac_folder, output_file):
    """
    ...
    with open(PAC_LIST, encoding="utf-8-sig") as f:
        ...
        for row in reader:
            ...
            if not os.path.exists(pac_file):
                ...
                for url in urls:
                    results.append([pac_name, url, "PAC_FILE_NOT_FOUND", "N/A"])
                continue
            ...
            for url in urls:
                ...
                results.append([pac_name, url, decision, status])
                print(f"    {url} -> {status}")
                
    def write_csv(filename):
        with open(filename, "w", newline="") as out:
            writer = csv.writer(out)
            writer.writerow(["pac_name", "test_url", "pac_decision", "http_status"])
            writer.writerows(results)
            
    success, final_file = safe_write_file(output_file, write_csv)
    if success:
        print(f"  ✓ Header test completed → {final_file}")
    return success
...
def compare_header_results(before, after, output_full, output_summary):
    ...
    with open(after) as f:
        for r in csv.DictReader(f):
            ...
            if not b:
                change = "NEW_ENTRY"
                changes_count += 1
            elif (b["pac_decision"] != r["pac_decision"]) or (b["http_status"] != r["http_status"]):
                change = "CHANGED"
                changes_count += 1
            else:
                change = "NO_CHANGE"
            ...
            row_data = [
                r["pac_name"], r["test_url"],
                b["pac_decision"] if b else "N/A", r["pac_decision"],
                b["http_status"] if b else "N/A", r["http_status"],
                change
            ]
            all_rows.append(row_data)
            if change != "NO_CHANGE":
    ...
    def write_full(filename):
        with open(filename, "w", newline="") as out:
            writer = csv.writer(out)
            writer.writerow(["pac_name", "test_url", "before_decision", "after_decision", "before_status", "after_status", "change"])
            writer.writerows(all_rows)
...
def workflow_setup_baseline():
    ...
    if not run_sanity_check_on_folder(OLD_PAC_DIR, "before_sanity_check.csv"):
...
def workflow_test_and_compare():
    ...
    if not run_sanity_check_on_folder(UPDATED_PAC_DIR, "after_sanity_check.csv"):
        print("\n❌ Workflow failed at AFTER sanity test")
        return
        
    if not compare_sanity_results("before_sanity_check.csv", "after_sanity_check.csv", "pac_sanity_comparison_summary.csv"):
...
def workflow_setup_baseline_header_only():
    ...
    if not run_header_test_from_folder(OLD_PAC_DIR, "before_header_result.csv"):
...
def workflow_test_and_compare_header_only():
    ...
    if not os.path.exists("before_header_result.csv"):
        print("\n❌ Baseline header results not found!")
    ...
    if not run_header_test_from_folder(UPDATED_PAC_DIR, "after_header_result.csv"):
        print("\n❌ Workflow failed at AFTER header test")
        return
        
    if not compare_header_results(
        "before_header_result.csv",
        "after_header_result.csv",
        "pac_header_comparison_full.csv",
        "pac_header_comparison_summary.csv"
    ):
    ...
    files_to_check = [
        {
            "name": PAC_LIST,
            "required": True,
            "expected_headers": ["pac_name", "pac_path"],
            "description": "list of PAC files to download and validate",
            "example_rows": [
                ["example_pac", "https://example.com/proxy.pac"]
            ]
        },
        {
            ...
            if not os.path.exists("updated_test_logic.csv"):
                ...
                if success:
                    ...
                    if os.path.exists(before_file):
                        ...
                        print("  Run Option 12 first to generate the comparison files.")
        elif choice == "14":
