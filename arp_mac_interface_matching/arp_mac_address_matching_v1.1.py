...
Enhanced with show run interface description parsing and fixed field structure parsing.
"""
...
class CiscoNetworkAnalyzer:
    ...
    def create_combined_report(self, arp_files: Dict, mac_files: Dict, interface_files: Dict) -> None:
        """Create a combined CSV report with matched information - handles multiple MACs per interface."""
        ...
        # Print debug info about multiple MACs per port
        print("\nDEBUG: Multiple MAC Analysis")
        multiple_mac_ports = {k: v for k, v in mac_by_device_port.items() if len(v) > 1}
        print(f"Ports with multiple MAC addresses: {len(multiple_mac_ports)}")
        for port_key, mac_list in multiple_mac_ports.items():
            device, port = port_key.split('_', 1)
            print(f"  {device} - {port}: {len(mac_list)} MACs")
            for mac_entry in mac_list:
                print(f"    - {mac_entry.mac} (VLAN {mac_entry.vlan}, Type: {mac_entry.entry_type})")
            ...
            if port_mac_entries:
                ...
                for mac_entry in port_mac_entries:
                    row = {
                        ...
                        'MAC_VLAN': mac_entry.vlan,  # Added MAC-specific VLAN
                        'IP_Address': '',
                        'Hostname': '',
                        'ARP_Age': ''
                    }
                ...
                # Interface with no MAC addresses - still include it
                row = {
                    'Device_Name': interface.device_name,
                    'Port': interface.port,
                    'Port_Name': interface.name,
        ...
        # Summary of multiple MAC addresses
