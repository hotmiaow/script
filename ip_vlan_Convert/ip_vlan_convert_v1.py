...
def cross_check_missing_mappings(
    ...
    for func in sorted(unique_functions):
        if func not in mapping or not mapping[func].strip():
            # Also check if any devices with this function have full-device mappings
...
if HAS_TKINTER:
    class MappingHelperDialog(tk.Toplevel):
        ...
        def _create_widgets(self):
            ...
            desc_lbl = ttk.Label(header_frame, text="Device names are parsed as <location>-<function><numbering>. Edit or batch fill missing VLAN types below.", foreground="#555")
            ...
            self.batch_entry = ttk.Combobox(tool_frame, textvariable=self.batch_vlan_var, values=["server vlan", "user vlan", "management vlan", "voice vlan", "dmz vlan", "transit vlan", "storage vlan"], width=18)
            ...
            self.tree.column("sample_devices", width=350, anchor=tk.W)
        ...
        def _on_double_click(self, event):
            ...
            entry = ttk.Combobox(self.tree, values=["server vlan", "user vlan", "management vlan", "voice vlan", "dmz vlan", "transit vlan", "storage vlan"])
        ...
        def _add_custom_key(self):
            ...
            type_entry = ttk.Combobox(top, textvariable=type_var, values=["server vlan", "user vlan", "management vlan", "voice vlan", "dmz vlan"])
    ...
    class AppGUI(tk.Tk):
        ...
        """
        def __init__(self, default_csv="ip_vlan.csv", default_mapping="mapping.csv"):
            super().__init__()
        ...
        def _create_ui(self):
            ...
            ttk.Entry(file_panel, textvariable=self.csv_path_var, width=55).grid(row=0, column=1, sticky=tk.EW, padx=8, pady=3)
            ...
            ttk.Entry(file_panel, textvariable=self.mapping_path_var, width=55).grid(row=1, column=1, sticky=tk.EW, padx=8, pady=3)
...
def run_cli_mapping_manager(mapping_path: str, mapping_data: Dict[str, str],
                            unique_functions: Set[str], function_to_devices: Dict[str, List[str]]) -> Dict[str, str]:
    """
    ...
    while True:
        ...
        if choice == "1":
            ...
            print("\nQuick VLAN Type Options: [1] server vlan, [2] user vlan, [3] management vlan, [4] voice vlan, [5] Custom text, [s] Skip")
            
            for k in list(missing_keys):
                samples = ", ".join(function_to_devices.get(k, []))
                ans = input(f"\nEnter VLAN type for '{k}' (samples: {samples}) [1/2/3/4/custom/s]: ").strip()
                if ans == "1":
                    mapping_data[k] = "server vlan"
                elif ans == "2":
                    mapping_data[k] = "user vlan"
                elif ans == "3":
                    mapping_data[k] = "management vlan"
                elif ans == "4":
                    mapping_data[k] = "voice vlan"
                elif ans.lower() == "s" or not ans:
                    continue
                else:
                    mapping_data[k] = ans
            ...
            if key:
                vtype = input(f"Enter VLAN type for '{key}': ").strip()
                if vtype:
...
def run_cli_mode(input_file: Optional[str] = None,
    ...
    if missing:
        ...
        if interactive and sys.stdin.isatty():
            ...
            if manage in ["", "y", "yes"]:
                mapping_data = run_cli_mapping_manager(mapping_file, mapping_data, unique_functions, function_to_devices)
    else:
        print(f"\n[OK] All {len(unique_functions)} device functions are mapped in {mapping_file}.")

    # 4. Perform Matching & Conversion
...
def main():
    ...
    if args.cli or not HAS_TKINTER:
        ...
        app = AppGUI(default_csv=default_in, default_mapping=default_map)
        app.mainloop()
